...
# 테스트 모드 설정 (기본값: true)
# 사용법: ./run-local.sh [test|prod]
TEST_MODE="${1:-test}"

if [[ "$TEST_MODE" == "prod" ]]; then
    TEST_MODE_VALUE="false"
    MODE_DISPLAY="프로덕션 모드 (실제 GPT API 호출)"
else
    TEST_MODE_VALUE="true"
    MODE_DISPLAY="테스트 모드 (고정된 응답 사용)"
fi
...
echo "================================================"
echo ""
echo -e "${BLUE}📝 실행 모드: ${MODE_DISPLAY}${NC}"
echo ""
...
echo "  포트: 8080"
echo "  모드: ${MODE_DISPLAY}"
echo "  모델: ${OPENAI_MODEL:-gpt-4o-mini}"
echo "================================================"
echo ""
...
# Spring Boot 실행 (테스트 모드 설정 포함)
./gradlew bootRun --args="--spring.profiles.active=local --app.test-mode=${TEST_MODE_VALUE}" \
    --console=plain
